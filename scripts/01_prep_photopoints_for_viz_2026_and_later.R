...
year = 2026
curr_panel = -1
# Have to update to full path, either on NETN server or local machine
orig_path <- paste0("./NETN/Monitoring_Projects/Freshwater_Wetland/", 
                     year, "/Photopoints/")
viz_path <- getwd()
...
sitedata <- read.csv(paste0(viz_path, '/data/Sentinel_and_USA-RAM_Sites_2025.csv'))
sitedata <- sitedata |> mutate(Panel = ifelse(Panel == 0, -1, Panel))
head(sitedata)
...
photos_new <- all_photos |> filter(grepl(paste0("_", year), file)) |> filter(!grepl("ID", file)) |> 
  filter(grepl("360L|270L|180L|090L", file))
...
photos_new$Label = substr(photos_new$file, 1, 4)
